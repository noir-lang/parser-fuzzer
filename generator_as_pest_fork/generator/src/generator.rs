...
use crate::cfg_generator::generate_cfg_generator;
...
pub(crate) fn generate(
    ...
    let cfg_generator = generate_cfg_generator(&rules);
...
fn generate_expr(expr: OptimizedExpr) -> TokenStream {
    match expr {
        ...
        OptimizedExpr::Choice(lhs, rhs, _weights) => {
            ...
            while let OptimizedExpr::Choice(lhs, rhs, _weights) = current {
                tail.push(generate_expr(*lhs));
                current = *rhs;
            }
            tail.push(generate_expr(current));
        ...
        OptimizedExpr::Weight => quote! { Ok(state) },
    }
}
