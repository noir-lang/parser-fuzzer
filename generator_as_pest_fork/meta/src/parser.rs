...
impl<'i> ParserNode<'i> {
    ...
    fn find_weight(&self) -> Option<u64> {
        match self.expr {
            ParserExpr::PosPred(..) => {
                None
            }
            ...
            ParserExpr::Seq(ref lhs, ref rhs) => {
                let w_lhs = lhs.find_weight();
                let w_rhs = rhs.find_weight();
                match (w_lhs, w_rhs) {
                    (Some(_), Some(_)) => {
                        panic!("weight specified multiple times");
                    }
            ...
            ParserExpr::Choice(ref lhs, ref rhs) => {
                let w0 = lhs.find_weight().unwrap_or(1);
                let w1 = rhs.find_weight().unwrap_or(1);
                Some(w0 + w1)
            }
            ParserExpr::Rep(..) => {
                None
...
pub enum ParserExpr<'i> {
    ...
    /// Specifies a weight for the production.
    Weight(u64),
}
...
fn convert_node(node: ParserNode<'_>) -> Expr {
    match node.expr {
        ...
        ParserExpr::Choice(node1, node2) => {
            let w0 = node1.find_weight().unwrap_or(1);
            let w1 = node2.find_weight().unwrap_or(1);
            Expr::Choice(
                Box::new(convert_node(*node1)),
                Box::new(convert_node(*node2)),
                (w0, w1)
            ...
            other_rule => {
                let node = match other_rule {
                    ...
                    Rule::weight => {
                        ...
                        // let s = format!("{:?}", pair);
                        let mut pairs = pair.into_inner();
                        let w_pair = pairs.next().expect("failed to parse weight");
                        // let start = unescape(pair.as_str()).expect("incorrect char literal");
                        let start_pos = w_pair.clone().as_span().start_pos();
                        let _colon_pair = pairs.next();
                        let weight_pair = pairs.next().unwrap();
                        // let weight_pair_s = format!("{:?}", weight_pair);
                        let weight: u64 = weight_pair.as_str().trim().parse().expect("failed to parse weight");
                        let end_pos = weight_pair.clone().as_span().end_pos();
                        ParserNode {
                            expr: ParserExpr::Weight(weight),
                            span: start_pos.span(&end_pos),
                        }
                    }
