...
pub fn factor(rule: Rule) -> Rule {
    ...
    Rule {
        ...
        expr: expr.map_top_down(|expr| {
            match expr {
                Expr::Choice(lhs, rhs, weights) => match (*lhs, *rhs) {
                    (Expr::Seq(l1, r1), Expr::Seq(l2, r2)) => {
                        if l1 == l2 {
                            Expr::Seq(l1, Box::new(Expr::Choice(r1, r2, weights)))
                        } else {
                            Expr::Choice(Box::new(Expr::Seq(l1, r1)), Box::new(Expr::Seq(l2, r2)), weights)
                        }
                    }
                        ...
                        if matches!(ty, RuleType::Atomic | RuleType::CompoundAtomic) =>
                    {
                        if *l1 == r {
                            ...
                            Expr::Choice(Box::new(Expr::Seq(l1, l2)), Box::new(r), weights)
                        }
                    }
                    ...
                    (l, Expr::Seq(r1, r2)) => {
                        if l == *r1 {
                            ...
                            Expr::Choice(Box::new(l), Box::new(Expr::Seq(r1, r2)), weights)
                        }
                    }
                    (lhs, rhs) => Expr::Choice(Box::new(lhs), Box::new(rhs), weights),
                },
                expr => expr,
            }
        }),
