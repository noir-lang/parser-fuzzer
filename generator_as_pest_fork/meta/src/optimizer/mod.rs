...
macro_rules! box_tree {
    ( Choice( $(                      $child:ident( $($args:tt)* )     ),+ ) ) => (
      Choice      ( $( Box::new( box_tree!( $child      ( $($args   )* ) ) ) ),+ , (1, 1) )
    );
...
fn rule_to_optimized_rule(rule: Rule) -> OptimizedRule {
    fn to_optimized(expr: Expr) -> OptimizedExpr {
        match expr {
            ...
            Expr::Choice(lhs, rhs, weights) => {
                OptimizedExpr::Choice(Box::new(to_optimized(*lhs)), Box::new(to_optimized(*rhs)), weights)
            }
            ...
            Expr::Weight => OptimizedExpr::Weight,
        }
    }
...
pub enum OptimizedExpr {
    ...
    Choice(Box<OptimizedExpr>, Box<OptimizedExpr>, (u64, u64)),
        ...
        F: FnMut(OptimizedExpr) -> OptimizedExpr,
    {
            ...
            F: FnMut(OptimizedExpr) -> OptimizedExpr,
        {
            ...
            match expr {
                ...
                OptimizedExpr::Choice(lhs, rhs, weights) => {
                    let mapped_lhs = Box::new(map_internal(*lhs, f));
                    let mapped_rhs = Box::new(map_internal(*rhs, f));
                    OptimizedExpr::Choice(mapped_lhs, mapped_rhs, weights)
                }
...
impl OptimizedExprTopDownIterator {
    ...
    fn iterate_expr(&mut self, expr: OptimizedExpr) {
        ...
        match expr {
            ...
            OptimizedExpr::Choice(lhs, rhs, _weights) => {
                self.right_branches.push(*rhs);
                self.next = Some(*lhs);
            }
            OptimizedExpr::PosPred(expr)
